...
wage104_url$`大學-薪資`<-gsub("—|…"," ",wage104_url$`大學-薪資`)
wage107_url$`大學-薪資`<-gsub("—|…"," ",wage107_url$`大學-薪資`)
wage104_url$`大學-薪資`<-as.numeric(wage104_url$`大學-薪資`)
wage107_url$`大學-薪資`<-as.numeric(wage107_url$`大學-薪資`)

#Join 104和107年度表格
wage_table<-left_join(wage104_url,wage107_url,by="大職業別")

#計算107,104薪資提高比率並新增到欄位'大學薪資成長'
wage_table$大學薪資成長<-wage107_url$`大學-薪資`/wage104_url$`大學-薪資`
...
```{r likeGrad}
#這是R Code Chunk
favorite_job$`研究所及以上-薪資`<-gsub("—"," ",favorite_job$`研究所及以上-薪資`)
